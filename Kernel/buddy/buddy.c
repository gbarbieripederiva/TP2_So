...
static void *giveMeMemoryWrapped(uint64_t size, uint32_t chunk, uint64_t currentSize, uint64_t memLocation)
{
    ...
    if (size <= currentSize / 2 && chunk < CHUNKS)
    {
        ...
        /*
            ...
            Should change if possible to get the smallest partially occupied chunk, but that's for later
        */
...
{
    if (OCCUPIED == status[0] || size > INITIAL_MEM_SIZE)
    { //either the space requested is too big or the memory is completely occupied
        return (void *)NULL;
    }
